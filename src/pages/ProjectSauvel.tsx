    ...
    <div className="md:hidden w-[calc(100%+3rem)] -mx-6 mb-8 relative">
          ...
          {images.map((img, index) => (
            ...
            >
              <div className="relative w-full overflow-hidden border border-[#2A2D35]/30 bg-[#0a0a0c] active:scale-[0.98] transition-transform duration-300 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                ...
                <TechLabel
                  text={`IMG_${String(index + 1).padStart(3, "0")}`}
                  className="absolute bottom-4 left-4 text-white bg-[#2959A0]/40 border border-[#2959A0]/50 backdrop-blur-sm"
                />
              </div>
            </div>
          ))}
      ...
      >
        <div className="bg-background/90 backdrop-blur px-4 py-2 border border-[#2A2D35]/50 flex items-center gap-2 shadow-2xl rounded-xs">
          <span className="font-mono text-[9px] text-[#2959A0] uppercase tracking-[0.2em] whitespace-nowrap">
            {(t as any).common.swipeHint}
          </span>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#2959A0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
        </div>
      </div>
      ...
      <div className="mt-8 flex justify-center items-center gap-1.5">
        {images.map((_, i) => (
          <div 
            key={i} 
            className={`h-1 transition-all duration-300 ${i === selectedIndex ? "w-4 bg-[#2959A0]" : "w-1 bg-[#2A2D35]/50"}`}
          />
        ))}
      </div>
    </div>
      ...
      <div className="min-h-screen pt-8 pb-4 md:py-12 px-6 md:px-16 lg:px-24 overflow-hidden flex flex-col items-center">
        ...
        <div className="w-full max-w-7xl flex flex-col md:grid md:grid-cols-1 lg:grid-cols-12 gap-4 md:gap-16 items-start mt-2 md:mt-0">
          ...
          <header className="lg:col-span-4 text-left relative z-20 md:sticky md:top-24 w-full">
            <TechLabel text={t.projects.items[1].tag} className="mb-4 md:mb-6 inline-block text-[9px] tracking-widest text-[#2959A0]" />
            ...
            <div className="w-12 h-px bg-[#2959A0] mb-6 md:mb-8" />
            ...
            <div className="mt-10 md:mt-12 flex flex-col gap-4">
               <div className="flex items-center gap-3">
                 <div className="w-1.5 h-1.5 rounded-full bg-[#2959A0]" />
                  <span className="font-mono text-[9px] md:text-[10px] text-muted-foreground/60 uppercase tracking-widest">{t.projects.items[1].type}</span>
               </div>
               <div className="flex items-center gap-3">
                 <div className="w-1.5 h-1.5 rounded-full bg-[#2A2D35]" />
                  <span className="font-mono text-[9px] md:text-[10px] text-muted-foreground/60 uppercase tracking-widest">{t.projects.items[1].focus}</span>
               </div>
            </div>
          </header>
          ...
          <div className="hidden md:block lg:col-span-8 relative min-h-[700px] md:min-h-[1100px] w-full">
